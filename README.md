...
* `STATIC_DATA` - статические данные — все, что объявлено в коде как буквальное. здесь НЕ хранятся входные данные, а только литеральные данные в коде.
    ...
    Итак, первый бит числа равен 0 (максимальное значение равно 2 ^ 31 - 1). Александр Владимирович can be the most intelligent person that I studied, but Computer
  architecture also the worst course that I studied at ITMO...
